        ...
        if (authHeader != null){
            ...
            if (authHeaderArr.length > 1 && authHeaderArr[1] != null){
                ...
                try {
                    ...
                    httpRespone.sendError(HttpStatus.FORBIDDEN.value(),"invalid/expired token");
                    return;
                }
            }else {
                httpRespone.sendError(HttpStatus.FORBIDDEN.value(),"Authorization token must be Beater [token]");
                return;
            }
        }else{
            httpRespone.sendError(HttpStatus.FORBIDDEN.value(),"Authorization token must be provided");
            return;
        }
